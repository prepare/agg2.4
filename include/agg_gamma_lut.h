...
namespace agg {
  ...
  //{{{
  template<class LoResT=int8u, class HiResT=int8u, unsigned GammaShift=8, unsigned HiResShift=8> class gamma_lut {
  public:
  ...
  template<class LinearType> class sRGB_lut;
  //{{{
  template<> class sRGB_lut<float> {
  public:
    //{{{
    sRGB_lut()
    {
  ...
  //}}}
  //{{{
  template<> class sRGB_lut<int16u> {
  public:
      //{{{
      sRGB_lut() {
        // Generate lookup tables.
  ...
  //{{{
  // Common base class for sRGB_conv objects. Defines an internal sRGB_lut object so that users don't have to.
  template<class T> class sRGB_conv_base {
  public:
    //{{{
  ...
  template<class T> sRGB_lut<T> sRGB_conv_base<T>::lut;

  // Wrapper for sRGB-linear conversion.
  // Base template is undefined, specializations are provided below.
  template<class T> class sRGB_conv;
  //{{{
  template<> class sRGB_conv<float> : public sRGB_conv_base<float>
  {
  public:
  ...
  //}}}
  //{{{
  template<> class sRGB_conv<int16u> : public sRGB_conv_base<int16u>
  {
  public:
  ...
  //}}}
  //{{{
  template<> class sRGB_conv<int8u> : public sRGB_conv_base<int8u>
  {
  public:
